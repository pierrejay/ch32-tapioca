...
//
//   node app/inline.mjs           regenerate index.html's inlined blocks
//   node app/inline.mjs --check   exit 1 if index.html is stale (used by run_all.sh)
...
const FILES = ['test/cobs.js', 'test/mdio_codec.js'];       // inlined in this order
...
if (check) {
  if (out !== orig) { console.error('✗ index.html inlined codec is OUT OF SYNC with test/*.js — run `node app/inline.mjs`'); process.exit(1); }
